...
export class BrandService {
    ...
    async updateBrandService(id: string, updateBrandDto: UpdateBrandDto): Promise<Brand> {
        try {
            // Check for duplicate brand name
            const brandDuplicate = await this.brandModel.findOne({ name: updateBrandDto.name }).exec();
